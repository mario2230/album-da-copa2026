import { ref, computed, onMounted } from "vue"

import {
  listJogadores,
  atualizarColetada
} from "@/services/database"


const stickersData = [
  // BRASIL
  {
    id: 1,
    nome: "Neymar Jr",
    selecao: "Brasil",
    foto: "https://img.a.transfermarkt.technology/portrait/big/68290-1697056482.jpg"
  },
  {
    id: 2,
    nome: "Vinicius Jr",
    selecao: "Brasil",
    foto: "https://img.a.transfermarkt.technology/portrait/big/371998-1664869583.jpg"
  },
  {
    id: 3,
    nome: "Rodrygo",
    selecao: "Brasil",
    foto: "https://img.a.transfermarkt.technology/portrait/big/412363-1697056493.jpg"
  },
  {
    id: 4,
    nome: "Alisson",
    selecao: "Brasil",
    foto: "https://img.a.transfermarkt.technology/portrait/big/105470-1668528909.jpg"
  },
  {
    id: 5,
    nome: "Marquinhos",
    selecao: "Brasil",
    foto: "https://img.a.transfermarkt.technology/portrait/big/181767-1668528947.jpg"
  },
  // ARGENTINA
  {
    id: 6,
    nome: "Lionel Messi",
    selecao: "Argentina",
    foto: "https://img.a.transfermarkt.technology/portrait/big/28003-1710080339.jpg"
  },
  {
    id: 7,
    nome: "Julián Álvarez",
    selecao: "Argentina",
    foto: "https://img.a.transfermarkt.technology/portrait/big/576024-1683633619.jpg"
  },
  {
    id: 8,
    nome: "Lautaro Martínez",
    selecao: "Argentina",
    foto: "https://img.a.transfermarkt.technology/portrait/big/406625-1695024989.jpg"
  },
  {
    id: 9,
    nome: "Emiliano Martínez",
    selecao: "Argentina",
    foto: "https://img.a.transfermarkt.technology/portrait/big/111873-1668180825.jpg"
  },
  // FRANÇA
  {
    id: 10,
    nome: "Kylian Mbappé",
    selecao: "França",
    foto: "https://img.a.transfermarkt.technology/portrait/big/342229-1682683695.jpg"
  },
  {
    id: 11,
    nome: "Antoine Griezmann",
    selecao: "França",
    foto: "https://img.a.transfermarkt.technology/portrait/big/125781-1663672271.jpg"
  },
  {
    id: 12,
    nome: "Ousmane Dembélé",
    selecao: "França",
    foto: "https://img.a.transfermarkt.technology/portrait/big/288230-1682683700.jpg"
  },
  // PORTUGAL
  {
    id: 13,
    nome: "Cristiano Ronaldo",
    selecao: "Portugal",
    foto: "https://img.a.transfermarkt.technology/portrait/big/8198-1694609670.jpg"
  },
  {
    id: 14,
    nome: "Bruno Fernandes",
    selecao: "Portugal",
    foto: "https://img.a.transfermarkt.technology/portrait/big/240306-1683882766.jpg"
  },
  {
    id: 15,
    nome: "Bernardo Silva",
    selecao: "Portugal",
    foto: "https://img.a.transfermarkt.technology/portrait/big/241641-1682581546.jpg"
  },
  // ALEMANHA
  {
    id: 16,
    nome: "Jamal Musiala",
    selecao: "Alemanha",
    foto: "https://img.a.transfermarkt.technology/portrait/big/580195-1680180859.jpg"
  },
  {
    id: 17,
    nome: "Kai Havertz",
    selecao: "Alemanha",
    foto: "https://img.a.transfermarkt.technology/portrait/big/309400-1682582021.jpg"
  },
  // INGLATERRA
  {
    id: 18,
    nome: "Harry Kane",
    selecao: "Inglaterra",
    foto: "https://img.a.transfermarkt.technology/portrait/big/132098-1681379945.jpg"
  },
  {
    id: 19,
    nome: "Jude Bellingham",
    selecao: "Inglaterra",
    foto: "https://img.a.transfermarkt.technology/portrait/big/581678-1693987944.jpg"
  },
  {
    id: 20,
    nome: "Bukayo Saka",
    selecao: "Inglaterra",
    foto: "https://img.a.transfermarkt.technology/portrait/big/433177-1667835061.jpg"
  },
  // ESPANHA
  {
    id: 21,
    nome: "Pedri",
    selecao: "Espanha",
    foto: "https://img.a.transfermarkt.technology/portrait/big/683840-1667830742.jpg"
  },
  {
    id: 22,
    nome: "Gavi",
    selecao: "Espanha",
    foto: "https://img.a.transfermarkt.technology/portrait/big/646740-1667830671.jpg"
  },
  {
    id: 23,
    nome: "Lamine Yamal",
    selecao: "Espanha",
    foto: "https://img.a.transfermarkt.technology/portrait/big/937958-1699476962.jpg"
  }
]

export function useAlbum() {

  const stickers = ref<any[]>([])
  const pesquisa = ref("")
  const filtro = ref("todas")

  async function carregarAlbum() {
    try {
      console.log("Carregando álbum com dados estáticos embutidos...")
    
      const jogadores = await listJogadores()
      console.log("Jogadores do banco:", jogadores)


      stickers.value = stickersData.map(sticker => {
        const jogador = jogadores.find((j: any) => j.id === sticker.id)
        return {
          ...sticker,
          coletada: jogador ? jogador.coletada === 1 : false
        }
      })
      console.log("Figurinhas carregadas:", stickers.value.length)
    } catch (error) {
      console.error("Erro ao carregar álbum:", error)
    
      stickers.value = stickersData.map(sticker => ({
        ...sticker,
        coletada: false
      }))
    }
  }

  async function marcarColetada(id: number) {
    const figurinha = stickers.value.find(sticker => sticker.id === id)
    if (!figurinha) {
      console.warn("Figurinha não encontrada:", id)
      return
    }

    const novoStatus = !figurinha.coletada
    try {
      await atualizarColetada(id, novoStatus)
      figurinha.coletada = novoStatus
      console.log(`Figurinha ${id} agora ${novoStatus ? 'coletada' : 'pendente'}`)
    } catch (error) {
      console.error("Erro ao atualizar status no banco:", error)

    }
  }

  const stickersFiltradas = computed(() => {
    let resultado = stickers.value

    if (pesquisa.value) {
      const termo = pesquisa.value.toLowerCase()
      resultado = resultado.filter(sticker =>
        sticker.nome.toLowerCase().includes(termo) ||
        sticker.selecao.toLowerCase().includes(termo)
      )
    }

    if (filtro.value === "coletadas") {
      resultado = resultado.filter(sticker => sticker.coletada)
    } else if (filtro.value === "pendentes") {
      resultado = resultado.filter(sticker => !sticker.coletada)
    }

    return resultado
  })

  const totalFigurinhas = computed(() => stickers.value.length)
  const totalColetadas = computed(() => stickers.value.filter(sticker => sticker.coletada).length)

  onMounted(() => {
    carregarAlbum()
  })

  return {
    pesquisa,
    filtro,
    marcarColetada,
    stickersFiltradas,
    totalFigurinhas,
    totalColetadas
  }
}