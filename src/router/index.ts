...
import { usuarioLogado } from "@/composables/useAuth.js";
...

  {
    ...
    children: [
...

      {
        ...
        component: () => import("@/views/ContatoCadastroPage.vue"),
      },

      {
        path: "contato-lista",
        component: () => import("@/views/ContatoListPage.vue"),
      },
    ],
  },
];
...
router.beforeEach((to, from, next) => {
  ...
  const logado = !!usuarioLogado.value;

  if (!rotaPublica && !logado) {
    return next("/login");
  }

  if (logado && (to.path === "/login" || to.path === "/cadastro")) {
    return next("/pages/album");
  }
